...
本文档说明如何把 DataSentry API 作为 `data1` 本机 loopback 服务运行，并让容器化 Alertmanager 通过 Docker bridge 回调。开发、提交和版本管理仍以本地仓库和 GitHub 为准；云端只运行明确 Git 版本。
...
- 当 Alertmanager 在 Docker 容器内运行时，增加仅绑定 `172.17.0.1:18000` 的 socket proxy，转发到 `127.0.0.1:18000`。
- 让容器化 Alertmanager 调用 `http://host.docker.internal:18000/api/alertmanager/webhook`。
- 使用固定只读工具执行巡检。
...
deploy/systemd/datasentry-alertmanager-proxy.socket.example
deploy/systemd/datasentry-alertmanager-proxy.service.example
...
如果 Alertmanager 直接运行在宿主机，可将 DataSentry receiver 指向：
...
如果 Alertmanager 运行在 Docker 容器内，容器中的 `127.0.0.1` 不是宿主机 loopback。此时不要把 DataSentry API 改成 `0.0.0.0`；安装一个只绑定 Docker bridge 的 socket proxy：

```bash
sudo install -o root -g root -m 0644 deploy/systemd/datasentry-alertmanager-proxy.socket.example /etc/systemd/system/datasentry-alertmanager-proxy.socket
sudo install -o root -g root -m 0644 deploy/systemd/datasentry-alertmanager-proxy.service.example /etc/systemd/system/datasentry-alertmanager-proxy.service
sudo systemctl daemon-reload
sudo systemctl enable --now datasentry-alertmanager-proxy.socket
systemctl status datasentry-alertmanager-proxy.socket
curl -fsS http://172.17.0.1:18000/api/health
```

容器化 Alertmanager 的 receiver 保持或改为：

```yaml
webhook_configs:
  - url: http://host.docker.internal:18000/api/alertmanager/webhook
```

变更前备份 Alertmanager 配置。变更后先检查 readiness，再做 smoke，并通过 Alertmanager API 投递一条测试告警确认真实回调返回 200。
